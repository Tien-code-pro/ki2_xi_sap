...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($name_err)&& empty($number_err)&& empty($address_err)&& empty($f_err)&& empty($location_err)&& empty($start_date_err)&& empty($end_date_err)){
        $sql = "INSERT INTO people (name, number, address,f, location,start_date,end_date) VALUES (?, ?, ?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt,"sssssss",$param_name,$param_number,$param_address,$param_f,$param_location,$param_start_date,$param_end_date);
