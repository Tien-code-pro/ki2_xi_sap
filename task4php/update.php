...
$name = $number = $address  = $f =$location=$start_date=$end_date="";
$name_err = $number_err = $address_err = $f_err = $location_err = $start_date_err = $end_date_err ="";
...
if (isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if (empty($input_number)){
        $address_err = 'Please enter an number';
    } else{
        $number =$input_number;
    }
    ...
    if(empty($name_err)&&empty($address_err)&&empty($salary_err)){
        $sql = "UPDATE people SET name = ?,number =?, address=?, f=?,location=?,start_date=?,end_date=? WHERE id=?";

        if($stmt = mysqli_prepare($link,$sql)){
            mysqli_stmt_bind_param($stmt,"sssssssi",$param_name,$param_number,$param_address,$param_f,$param_location,$param_start_date,$param_end_date,$param_id);
